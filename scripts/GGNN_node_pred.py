...
# (Recommended) make undirected + add self-loops for GGNN stability
...
class GGNN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        ...
        self.ggnn = GatedGraphConv(hidden_channels, num_layers=3)
...
for seed in SEEDS:
    ...
    model = GGNN(x.shape[1], hidden_channels=64, out_channels=num_classes).to(device)
    data = data.to(device)
    ...
    acc, f1, auroc, auprc = eval_on(model, data, data.test_mask)
...
print("\n".join(summary))
with open(os.path.join(dataset_dir, "ggnn_node_results.txt"), "w") as f:
    f.write("\n".join(summary) + "\n")
