...
for seed in SEEDS:
    ...
    model = GGNN(x.shape[1], hidden_channels=64, out_channels=len(y_unique)).to(device)
    data = data.to(device)
...
for seed in SEEDS:
    ...
    # Train
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = GGNN(x.shape[1], hidden_channels=64, out_channels=len(unique_labels)).to(device)
    data = data.to(device)
    criterion = nn.CrossEntropyLoss(weight=class_weights.to(device))
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)
